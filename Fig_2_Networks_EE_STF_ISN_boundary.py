...
if b_trace:
    for u_max_idx in range(len(l_u_max)):
        ...
        for re_idx in range(len(l_re)):
            re = l_re[re_idx]
            string = """ {solve_for} /. Solve[{{ (1.0/{tau_e}) * ( (1 + {u_f} * {u_max} * {re} * {tau_u}) * Jee * {alpha_e} * (({re})^(({alpha_e}-1)/{alpha_e}))/(1 + {u_f} * {re} * {tau_u}) - 1) - 1/{tau_u} - {u_f} * {re}==0}}, {solve_for}, Reals]""".format(
...
if b_determinant:
    for u_max_idx in range(len(l_u_max)):
        ...
        for re_idx in range(len(l_re)):
            re = l_re[re_idx]
            string = """ {solve_for} /. Solve[{{ (1.0/{tau_e}) * (-(1/{tau_u}) * (1 + {u_f} * {u_max} * {re} * {tau_u}) * Jee * {alpha_e} * (({re})^(({alpha_e}-1)/{alpha_e}))/(1 + {u_f} * {re} * {tau_u}) - (1 + {u_f} * {u_max} * {re} * {tau_u}) * Jee * {alpha_e} * (({re})^((2 * {alpha_e}-1)/{alpha_e})) * {u_f} /(1 + {u_f} * {re} * {tau_u}) + 1/{tau_u} + {u_f} * {re} - Jee * {alpha_e} * (({re})^((2 * {alpha_e}-1)/{alpha_e})) * {u_f} * {u_max} + Jee * {alpha_e} * ({re})^((2*{alpha_e}-1)/{alpha_e}) * {u_f} * (1 + {u_f} * {u_max} * {re} * {tau_u})/(1 + {u_f} * {re} * {tau_u}))==0}}, {solve_for}, Reals]""".format(
                tau_e=tau_e, tau_u=tau_u, u_f=u_f, u_max=u_max, re=re, alpha_e=alpha_e, alpha_i=alpha_i,
                solve_for='{Jee}')
...
plt.legend([r'$u_{max}$: 2', r'$u_{max}$: 6', r'$u_{max}$: 10'], prop={"family": "Arial", 'size': font_size_1}, loc='best')
if b_zoomin:
    plt.savefig('paper_figures/png/Fig_2_Linear_networks_EE_STF_ISN_boundary_zoomin.png')
    plt.savefig('paper_figures/pdf/Fig_2_Linear_networks_EE_STF_ISN_boundary_zoomin.pdf')
else:
    plt.savefig('paper_figures/png/Fig_2_Linear_networks_EE_STF_ISN_boundary.png')
    plt.savefig('paper_figures/pdf/Fig_2_Linear_networks_EE_STF_ISN_boundary.pdf')
...
plt.legend([r'$u_{max}$: 2', r'$u_{max}$: 6', r'$u_{max}$: 10'], prop={"family": "Arial", 'size': font_size_1}, loc='best')
if b_zoomin:
    plt.savefig('paper_figures/png/Fig_2_Supralinear_networks_EE_STF_ISN_boundary_zoomin.png')
    plt.savefig('paper_figures/pdf/Fig_2_Supralinear_networks_EE_STF_ISN_boundary_zoomin.pdf')
else:
    plt.savefig('paper_figures/png/Fig_2_Supralinear_networks_EE_STF_ISN_boundary.png')
    plt.savefig('paper_figures/pdf/Fig_2_Supralinear_networks_EE_STF_ISN_boundary.pdf')
...
alpha_e, alpha_i = 2, 2
trace_boundary_value = np.zeros((len(l_tau_x), len(l_re))) * np.nan
...
# turning point part
for tau_x_idx in range(len(l_tau_x)):
    tau_x = l_tau_x[tau_x_idx]
    turning_points_trace_re.append((-(2 * tau_e + tau_x) + np.sqrt(
        4 * np.power(alpha_e, 2) * np.power(tau_e, 2) + 4 * np.power(alpha_e, 2) * tau_e * tau_x + np.power(tau_x,
                                                                                                            2))) / (
                                               2 * (alpha_e + 1) * tau_e * tau_x * u_d))
...
for i in range(len(turning_points_trace_re)):
    ...
    turning_points_trace_Jee.append((tau_e / tau_x + 1 + 2 * tau_e * u_d * trace_re + tau_e * tau_x * np.power(u_d,
                                                                                                               2) * np.power(
        trace_re, 2) + u_d * tau_x * trace_re) / (alpha_e * np.power(trace_re, (alpha_e - 1) / alpha_e)))

for j in range(len(turning_points_det_re)):
    tau_x = l_tau_x[j]
    det_re = turning_points_det_re[j]
    turning_points_det_Jee.append(
        np.power(1 + tau_x * u_d * det_re, 2) / (alpha_e * np.power(det_re, (alpha_e - 1) / alpha_e)))
    ...
    for re_idx in range(len(l_re)):
        ...
        if len(P) == 1:
            ...
            if solution_Jee != 'Global`Jee':
                ...
                trace_boundary_value[tau_x_idx, re_idx] = Jee_solution
            else:
                pass

det_boundary_value = np.zeros((len(l_tau_x), len(l_re))) * np.nan
...
for tau_x_idx in range(len(l_tau_x)):
    ...
    for re_idx in range(len(l_re)):
        ...
        if len(P) == 1:
            ...
            if solution_Jee != 'Global`Jee':
                Jee_solution = float(solution_Jee[1:])
                det_boundary_value[tau_x_idx, re_idx] = Jee_solution
            else:
                pass
...
ISN_boundary_value = np.zeros((len(l_tau_x), len(l_re))) * np.nan
for tau_x_idx in range(len(l_tau_x)):
    for re_idx in range(len(l_re)):
        if det_boundary_value[tau_x_idx, re_idx] < trace_boundary_value[tau_x_idx, re_idx]:
            ISN_boundary_value[tau_x_idx, re_idx] = det_boundary_value[tau_x_idx, re_idx]
        else:
            ISN_boundary_value[tau_x_idx, re_idx] = trace_boundary_value[tau_x_idx, re_idx]
...
# plt.plot(turning_points_det_re[0] * 200, turning_points_det_Jee[0], 'o', markerfacecolor=pal[0], markersize=marker_size)
# plt.plot(turning_points_det_re[1] * 200, turning_points_det_Jee[1], 'o', markerfacecolor=pal[1], markersize=marker_size)
# plt.plot(turning_points_det_re[2] * 200, turning_points_det_Jee[2], 'o', markerfacecolor=pal[2], markersize=marker_size)
...
plt.legend([r'$\tau_x$: 0.2', r'$\tau_x$: 0.4', r'$\tau_x$: 0.6'], prop={"family": "Arial", 'size': font_size_1}, loc='best')
plt.savefig('paper_figures/png/Fig_S3_Supralinear_networks_EE_STD_ISN_boundary_turning_point_tau_x.png')
plt.savefig('paper_figures/pdf/Fig_S3_Supralinear_networks_EE_STD_ISN_boundary_turning_point_tau_x.pdf')
